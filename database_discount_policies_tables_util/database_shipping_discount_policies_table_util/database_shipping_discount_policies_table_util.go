package database_shipping_discount_policies_table_util
...
const (
	tableName                   = discountPoliciesTablesConst.ShippingDiscountPoliciesTableName
	codeColumnName              = discountPoliciesTablesConst.ShippingDiscountPoliciesCodeColumnName
	minimumOrderPriceColumnName = discountPoliciesTablesConst.ShippingDiscountPoliciesMinimumOrderPriceColumnName
)
...
const (
	queryCreateTable = "CREATE TABLE IF NOT EXISTS " + tableName + util.EndOfLine +
		...
		minimumOrderPriceColumnName + "	DECIMAL(30, 2)	NOT NULL," + util.EndOfLine +
...
// InsertDiscountPolicy inserts the given discount policy to the database table `shipping_discount_policies`.
func InsertDiscountPolicy(discountPolicy DiscountPolicy, databasePtr *sql.DB) Status {
	return database_util.PrepareThenExecuteQuery(databasePtr, queryInsertDiscountPolicy, discountPolicy.Code, discountPolicy.ShippingMinimumOrderPrice)
}
